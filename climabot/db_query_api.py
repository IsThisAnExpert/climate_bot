...
group = parser.add_mutually_exclusive_group(
    required=True
)  # add mutually exclusive required args
parser.add_argument(
    "-d",
    "--mariadb_group",
...
group.add_argument(
    "-f",
    "--csv_file",
    help="path to the csv file with the twitter handles to parse in the first (0) column",
)
...
def fill_database(user):
    ...
    created_at = datetime.strftime(
        datetime.strptime(user_data["created_at"], "%a %b %d %H:%M:%S +0000 %Y"),
        "%Y-%m-%d %H:%M:%S",
    )
